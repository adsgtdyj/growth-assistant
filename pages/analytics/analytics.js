...
Page({
  ...
  _compute() {
    ...
    const itemStats = this._buildItemStats(habits, checkins);
  ...
  // 统计每个习惯下 plan.items 各项被勾选的次数
  _buildItemStats(habits, checkins) {
    const result = [];
    habits.forEach(h => {
      const items = (h.plan && Array.isArray(h.plan.items)) ? h.plan.items : [];
      if (items.length === 0) return;
      const counts = {};
      items.forEach(it => { counts[it.text] = 0; });
      let total = 0;
      checkins.forEach(c => {
        if (c.habitId !== h.id) return;
        if (Array.isArray(c.selectedItems)) {
          c.selectedItems.forEach(si => {
            if (counts.hasOwnProperty(si.text)) counts[si.text]++;
          });
          if (c.selectedItems.length > 0) total++;
        }
      });
      const itemsView = items.map(it => ({
        text: it.text,
        color: it.color || '#6366f1',
        count: counts[it.text] || 0
      })).sort((a, b) => b.count - a.count);
      if (total === 0) return;  // 没勾选记录的习惯不显示
      result.push({
        habitId: h.id,
        habitName: h.name,
        habitIcon: store.habitEmoji(h.icon),
        habitColor: h.color || '#6366f1',
        items: itemsView,
        total: total
      });
    });
